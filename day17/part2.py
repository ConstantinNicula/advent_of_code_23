...
PREV_DIR_VERT, PREV_DIR_HOR = 0, 1

valid_moves = {
    PREV_DIR_HOR: [((-1, 0), i) for i in range(4, 11)] + [((1, 0), i) for i in range(4, 11)],
    PREV_DIR_VERT: [((0, -1), i) for i in range(4, 11)] + [((0, 1), i) for i in range(4, 11)]
}
...
def do_move(pos: tuple[int, int], move: tuple[tuple[int, int], int], grid: list[str])-> tuple[tuple[int, int], int, int]:
    dir, count = move 
    cost = 0
    cnt = 0
...
def solve(grid: list[str]):
    ...
    while not Q.empty():
        ...
        for move in valid_moves[prev_move]:
            v, cost, cnt = do_move(u, move, grid)
            if cnt < 4: 
                continue
